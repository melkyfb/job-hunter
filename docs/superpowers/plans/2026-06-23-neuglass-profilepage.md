# NeuGlass ProfilePage Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Apply hybrid Neumorphism + Glassmorphism design to ProfilePage — sticky glass ActionBar at top, all content sections in neumo-raised panels, textareas neumo-inset, blue palette replaces purple accent.

**Architecture:** Two-file change only. CSS custom properties added to `index.css` `:root` block provide the token layer. `ProfilePage.tsx` consumes those tokens via inline styles (existing pattern), restructuring layout so a sticky glass `ActionBar` floats over the background gradient while content panels extrude from it.

**Tech Stack:** React 19, TypeScript, Vite, CSS custom properties, inline styles (no new deps)

## Global Constraints

- No new npm dependencies
- Light mode only — `[data-theme="dark"]` block and `@media (prefers-color-scheme: dark)` block in `index.css` are NOT modified
- CSS tokens go inside existing `:root { }` block in `frontend/src/index.css`
- Inline styles only in components — no new CSS classes
- TypeScript strict: all style objects typed as `React.CSSProperties` or use `satisfies React.CSSProperties`
- `resize: 'vertical' as const` — needed because `'vertical'` is a union type in CSSProperties
- `WebkitBackdropFilter` key (capital W) for Safari prefix alongside `backdropFilter`
- No changes to backend, `App.tsx`, or any page other than `ProfilePage.tsx`
- `var(--mono)` already defined in `:root` as `ui-monospace, Consolas, monospace` — reference it in textarea styles
...
| File | Change |
|------|--------|
| `frontend/src/index.css` | Add NeuGlass token block inside `:root { }` after existing tokens |
| `frontend/src/pages/ProfilePage.tsx` | Restructure layout: PAGE_BG wrapper → glass ActionBar → neumo panels per section |

No files created. No backend changes.

---

### Task 1: NeuGlass CSS Tokens

**Files:**
- Modify: `frontend/src/index.css` (inside `:root { }` block, after line 12)

**Interfaces:**
- Consumes: nothing
- Produces: CSS custom properties consumed by Task 2:
  - `--neumo-bg`, `--neumo-shadow-dark`, `--neumo-shadow-light`
  - `--neumo-raised`, `--neumo-raised-sm`, `--neumo-inset`, `--neumo-pressed`
  - `--glass-bg`, `--glass-border`, `--glass-blur`, `--glass-shadow`
  - `--blue-primary`, `--blue-medium`, `--blue-light`, `--blue-border`, `--blue-gradient`
  - `--neumo-text`, `--neumo-text-s`

- [ ] **Step 1: Add token block to `:root`**

Open `frontend/src/index.css`. After the line `--shadow: ...;` (line 12) and before the `--sans:` line, insert this block:
...
The resulting `:root` block starts at line 1. The token block goes between `--shadow` and `--sans`. Verify the braces stay balanced.
...
Expected: no errors. CSS changes cannot introduce TS errors; this step confirms no accidental file corruption.

- [ ] **Step 3: Commit**

```bash
rtk git add frontend/src/index.css
rtk git commit -m "feat: add NeuGlass CSS custom property tokens to :root"
```

---

### Task 2: ProfilePage NeuGlass Layout

**Files:**
- Modify: `frontend/src/pages/ProfilePage.tsx` (full rewrite of layout; logic unchanged)

**Interfaces:**
- Consumes: CSS tokens from Task 1 (`--neumo-*`, `--glass-*`, `--blue-*`)
- Produces: nothing (leaf component, no new exports)

- [ ] **Step 1: Replace the file with the NeuGlass version**

Replace the full contents of `frontend/src/pages/ProfilePage.tsx` with:
...
- `'vertical'` → `'vertical' as const` (already in NEUMO_INSET above)
- `'uppercase'` → `'uppercase' as const` (already in SECTION_TITLE above)
- `'relative'` → `'relative' as const` (already in BTN_PRIMARY above)
- `'wrap'` → `'wrap' as const` (already in ACTION_BAR above)

- [ ] **Step 3: Commit**

```bash
rtk git add frontend/src/pages/ProfilePage.tsx
rtk git commit -m "feat: apply NeuGlass design to ProfilePage — glass ActionBar + neumo panels"
```
...
|-----------------|------------|
| Background `var(--blue-gradient)` on page wrapper | `PAGE_BG`, Task 2 |
| ActionBar glass, sticky z-50 | `ACTION_BAR`, Task 2 |
| All section cards neumo-raised | `NEUMO_PANEL` on each section, Task 2 |
| ExperienceCard neumo-raised-sm | `NEUMO_CARD_SM` wrapper, Task 2 |
| Textareas neumo-inset | `NEUMO_INSET` on `PromptEditor` textarea, Task 2 |
| Buttons `--neumo-raised-sm`, `:active` → `--neumo-pressed` | `onMouseDown`/`onMouseUp` on all buttons, Task 2 |
| `will-change: transform` on glass | `ACTION_BAR`, Task 2 |
| `WebkitBackdropFilter` Safari prefix | `ACTION_BAR`, Task 2 |
| Text `--neumo-text` / `--neumo-text-s` | All text spans, Task 2 |
| `--blue-primary` replaces `--accent` | Section titles, links, dots, badges, Task 2 |
| Focus visible `outline` for a11y | Missing — see note below |
| All 17 CSS tokens in `:root` | Task 1 |
| No new npm dependencies | Both tasks add 0 deps |
| Dark mode block untouched | Task 1 scoped to `:root` only |

**A11y gap:** Spec §6 requires `outline: 2px solid var(--blue-primary)` on `:focus-visible` for interactive elements. Inline styles cannot target pseudo-selectors. **Fix:** Add this CSS rule to `index.css` as part of Task 1:

```css
/* NeuGlass focus ring — neumo shadows alone don't communicate focus */
...
Add this block after the `:root { }` closing brace in `index.css`.

**Placeholder scan:** None found.

**Type consistency:** All style constants defined at module level and referenced verbatim in JSX — no name divergence.
